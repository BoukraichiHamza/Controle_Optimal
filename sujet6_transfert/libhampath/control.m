...
%  par  -  real vector, parameters given to control
...
strFinal = [str int2str(ceil(1e3*rand(1)))]; nomfilein=[ 'controlIn' strFinal '.txt'];
while(exist(nomfilein,'file'))
    strFinal = [str int2str(ceil(1e3*rand(1)))]; nomfilein=[ 'controlIn' strFinal '.txt'];
end;
nomfileout=[ 'controlOu' strFinal '.m']; nomfun='control'; nominterface='matlab'; nomfunint=[ 'controlOu' strFinal];
