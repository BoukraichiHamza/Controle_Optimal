...
strFinal = [str int2str(ceil(1e3*rand(1)))]; nomfilein=[ 'hvfunIn' strFinal '.txt'];
while(exist(nomfilein,'file'))
    strFinal = [str int2str(ceil(1e3*rand(1)))]; nomfilein=[ 'hvfunIn' strFinal '.txt'];
end;
nomfileout=[ 'hvfunOu' strFinal '.m']; nomfun='hvfun'; nominterface='matlab'; nomfunint=[ 'hvfunOu' strFinal];
